...
C              logic to determine which monthly file to open with that to
C              skip into the file.
C             -Change 'file not found' output to KONSOL (so error diagnostics
C              are all written to the same unit).
C             -Omit from declarations unused variables AGNR, EGNR, NEI,
C              CTN, CTNN, SSIN.
C             -Patch an underflow condition in subroutine ioncom (Danilov-
C              Yaichnikov ion composition model).
C             -Reduce ARGMAX to avoid underflow
C Stan Solomon's notes on earlier changes:
...
C          Declarations for Apr 94 changes
       CHARACTER*(*) CCIRNM,URSINM
       PARAMETER (NRMC=715 , NRMU=494)
C          NRMC = no recs per month in CCIR file = no. fields per month
C                 over no. fields per rec =  (13x76x2 + 9x49x2) / 4
C          NRMU = no recs per month in URSI file = (13x76x2) / 4
       CHARACTER*4 STUFF
      ...
      COMMON	/EUA_BLOCK1/HMF2,NMF2,HMF1	         /EUA_CONST/UMR
     &		/EUA_BLOCK2/B0,B1,C1      /EUA_BLOCK3/HZ,T,HST,STR
     &  	/EUA_BLOCK4/HME,NME,HEF   /EUA_BLOCK5/NIGHT,E
     &		/EUA_BLOCK6/HMD,NMD,HDX   /EUA_BLOCK7/D1,XKK,FP30,FP3U,FP1,FP2
     &  	/EUA_BLOCK8/HS,TNHS,XSM,MM,DTI,MXSM    	
     &  	/EUA_BLOTN/XSM1,TEXOS,TLBDH,SIGMA /EUA_BLOTE/AHH,ATE1,STTE,DTE
     &		/EUA_BLO10/BETA,ETA,DELTA,ZETA	 /EUA_ARGEXP/ARGMAX

ccc      EXTERNAL 		XE1,XE2,XE3,XE4,XE5,XE6,TEDER
...
C          Patch:  reduce max absolute value to avoid overflow/underflow
C                  when exponentiating; e.g., EXP(-87.425) underflows
	ARGMAX=87.0
C       ARGMAX=88.0
...
C!!!!!!! F-REGION PARAMETERS AND E-PEAK !!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      IF (URSIF2 .NEQV. URSIFO) GOTO 7790
...
C          Read CCIR coefficient set for chosen month....................
 ...
 7791 READ  (IUCCIR,'(A)',ERR=7797) STUFF
      READ  (IUCCIR,'(4E15.8)') F2,FM3
      CLOSE (IUCCIR)

C          Read URSI coefficient set for chosen month....................
      IF (URSIF2) THEN
	OPEN  (IUCCIR,FILE=URSINM,STATUS='OLD',ERR=7798)
	DO 7792 I=1,NRMU*(MONTH-1)
 7792   READ  (IUCCIR,'(A)',ERR=7799) STUFF
	READ  (IUCCIR,'(4E15.8)') F2
	CLOSE (IUCCIR)
      ENDIF
      URSIFO = URSIF2
      MONTHO = MONTH
...
c----------------------------------------------------------------
      COMMON	/EUA_BLOCK1/	HMF2,XNMF2,HMF1
     &		/EUA_BLO10/		BETA,ETA,DELTA,ZETA
     &		/EUA_ARGEXP/	ARGMAX
...
C ELECTRON DENSITY FOR THE BOTTOMSIDE F-REGION (HMF1...HMF2).                   
      COMMON	/EUA_BLOCK1/HMF2,XNMF2,HMF1
     &		/EUA_BLOCK2/B0,B1,C1	/EUA_ARGEXP/ARGMAX
      X=(HMF2-H)/B0
	z=x**b1
...
C ELECTRON DENSITY FOR THE INDERMEDIUM REGION (HEF..HZ).                        
      COMMON	/EUA_BLOCK3/	HZ,T,HST,STR
     &		/EUA_BLOCK4/	HME,XNME,HEF
...
C ELECTRON DENSITY FOR THE E AND VALLEY REGION (HME..HEF).   
      LOGICAL NIGHT   
      COMMON	/EUA_BLOCK4/	HME,XNME,HEF
     &  	/EUA_BLOCK5/	NIGHT,E(4)                    
      T3=H-HME        
...
C ELECTRON DENSITY FOR THE D REGION (HA...HME).    
      COMMON	/EUA_BLOCK4/	HME,XNME,HEF
     &		/EUA_BLOCK6/	HMD,XNMD,HDX
     &		/EUA_BLOCK7/	D1,XKK,FP30,FP3U,FP1,FP2    
...
C EXP-FUNCTION.
c----------------------------------------------------------------
      COMMON /EUA_BLOTE/AH(7),ATE1,ST(6),D(5)
C
...
c----------------------------------------------------------------
      REAL 		MM
      COMMON  /EUA_BLOCK8/  HS,TNHS,XSM(4),MM(5),G(4),M
...
C THE  HEIGHT ABOVE WHICH TN BEGINS TO BE DIFFERENT FROM TI                     
      COMMON	/EUA_BLOTN/XSM1,TEX,TLBD,SIG
...
C CHOSEN HEADER NUMBERS.FE(M) IS THE CORRESPONDING SET.                         
      DIMENSION RFE(4),FE(*),FIELD(*),EFE(4)     
      K=0             
100   DO 101 I=1,4    
      K=K+1           
101   EFE(I)=FIELD(K)                              
...
CCC       used to be IMO-1 which was just plain wrong
55		MONTH=IMO
		IDAY=IDOY-MOOLD
	RETURN
	END		 SUBROUTINE MODA
...
C          Patch to avoid underflow:
C3               cn(i) = cm(i) * exp( hx * (hx * alh(i) + beth(i)) )
 3               cn(i) = 0.
		 xpon =               hx * (hx * alh(i) + beth(i))
		 if (xpon .gt. -69.) cn(i) = cm(i) * exp (xpon)
