...
contains
  ...
  subroutine local_interstellar_spectrum_a(nP,MomentumSi_I, XyzSi_D, & ! Input
    ...
    real :: ADefault, ZDefault    ! Default values for A and Z if not present
    !--------------------------------------------------------------------------
    ADefault = 1.0; ZDefault = 1.0
    if(present(A)) ADefault = A
    if(present(Z)) ZDefault = Z

    ! p[Si] -> E_K[Si]
    EnergySi_I = sqrt((MomentumSi_I * cLightSpeed)**2 +         &
         (ADefault * cProtonMass * cLightSpeed**2)**2)
    ! E_K[Si] -> R(E_K)[Si]
    RigiditySi_I = ADefault / abs(ZDefault*cElectronCharge) *   &
    ...
    DistTimesP2Gn_I = 1.9E+4 * RigidityGV_I**(-2.78) /          &
         (1 + 0.4866 * RigidityGV_I**(-2.51))
    ! j_LIS[.../(GeV/nuc)] -> p^2*f|_{infty}[Si]
    DistTimesP2Si_I = DistTimesP2Gn_I/(ADefault * cGEV)
