...
def Alg(nLong,nLat,nParam, Param_I,Long_I,Lat_I,Br_C,
        ...
        GLRadiusRange_I, UsePIL, ARMag, UseCMEGrid, verbose=True):
   Long0     = Param_I[0]
   LongEarth = Param_I[1] 
   xPositive = Param_I[2]
   yPositive = Param_I[3]
   ...
   #udpated GL_Bstrength see: JIRA ROR-478
   #GL_Bstrength=GL_poloidal/(21.457435*GLRadius**4)  <--- previous version
   ...
   flux_rope_params = {}
   flux_rope_params['Latitude'] = GL_Latitude
   flux_rope_params['Longitude'] = GL_Longitude
   flux_rope_params['Orientation'] = GL_Orientation
   flux_rope_params['Radius'] = GLRadius
   flux_rope_params['Bstrength'] = GL_Bstrength
   flux_rope_params['Stretch'] = Stretch
   flux_rope_params['Distance'] = Distance
   flux_rope_params['Height'] = GLRadius + Distance - Stretch - 1.0
   flux_rope_params['Angular_size'] = 2*GLRadius/Distance*Rad2Deg
   flux_rope_params['Poloidal_flux'] = GL_poloidal
   flux_rope_params['xPositive'] = xPositive
   flux_rope_params['yPositive'] = yPositive
   flux_rope_params['xNegative'] = xNegative
   flux_rope_params['yNegative'] = yNegative
   flux_rope_params['xCenter'] = XyARCenter_D[0]
   flux_rope_params['yCenter'] = XyARCenter_D[1]
   ...
   if UseCMEGrid:
      ...
      flux_rope_params['R_Start'] = CMEbox_Start[0]
      flux_rope_params['R_End'] = CMEbox_End[0]
      flux_rope_params['Longitude_Start'] = CMEbox_Start[1]
      flux_rope_params['Longitude_End'] = CMEbox_End[1]
      flux_rope_params['Latitude_Start'] = CMEbox_Start[2]
      flux_rope_params['Latitude_End'] = CMEbox_End[2]
...
#   FileId.close()
   ...
   print 'before return Param_I:', Param_I

   flux_rope_params['PSizeMap_C'] = PSizeMap_C
   flux_rope_params['NSizeMap_C'] = NSizeMap_C
   flux_rope_params['xPIL'] = iXPIL_I
   flux_rope_params['yPIL'] = iYPIL_I
   

   
   return(nLong,nLat,nParam, Param_I,Long_I,Lat_I,Br_C,PSizeMap_C,NSizeMap_C, flux_rope_params)
